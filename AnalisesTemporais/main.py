...
class MyWindow(QScrollArea):
    ...
    def initUI(self):
        ...
        # Criando um QLabel
        self.titleAction = QLabel("Nome", self)
        ...
        # Criando um QLabel
        self.titleCountry = QLabel("País", self)
        ...
        # Criando um QLabel
        self.titleSector = QLabel("Setor", self)
        ...
        self.btnSubmit.setText("Pesquisar")
    ...
    def button_clicked(self):
        ...
        if self.existe == "TRUE":
            ...
            plt.figure(figsize=(4, 2))
            plt.plot(self.data[:1000], 'r-')
            plt.plot(self.data[1000:], 'g-')
            plt.xlabel('DIAS')
            plt.ylabel('VALOR R$')
            plt.title(self.actionName)
            plt.axvline(self.data.index[1000], 0, 30, color='k', linestyle='dashed', label='Teste')
